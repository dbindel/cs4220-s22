...
# ╔═╡ 531cc513-f721-4889-a525-fd7f2cf488f7
using LinearAlgebra

# ╔═╡ 07df1e1a-a931-41bc-9696-a3bbc9039598
using Plots

# ╔═╡ c90f8e16-3204-4d96-9a6c-4bffe9b1b3b8
md"""
# Notebook for 2022-04-01
"""

# ╔═╡ 44c2fa3a-d03e-4d10-ad6a-da550448c747
md"""
...
If $\phi : \mathbb{R}^n \rightarrow \mathbb{R}$ is $\mathcal{C}^2$ (i.e. if it is
...
# ╔═╡ 76eadee7-4f04-4b43-90ce-574303b09fb6
md"""
Let's give an illustration comparing a function $\phi$ to the first and second-order Taylor expansions about some point $x_0$.
"""

# ╔═╡ b1e5d1d5-3ced-4381-8838-6dbada1cf59d
begin
	...
	# Taylor expand about some random point, plot error over some range
	xx = range(-1.0, 1.0, length=100)
	x0 = 2*rand(2) .- 1.0
end

# ╔═╡ 7cdcb241-0b28-42f0-8b53-ed818881ef53
function plot_vs_approx(ϕ, ϕ̂, x0, xx, yy)

	# Plot function vs approximation

	p1 = plot(xx, xx, (x,y)->ϕ([x; y]), st=:contour)
	plot!([x0[1]], [x0[2]], marker=true, label="x0")
	plot!(xx, xx, (x,y)->ϕ̂([x; y]), st=:contour, style=:dash, colorbar=false)

	# Plot error
	p2 = plot(xx, xx, (x,y)->ϕ([x; y])-ϕ̂([x; y]), st=:contour)
	plot!([x0[1]], [x0[2]], marker=true, label="x0")

	plot(p1, p2, layout=(1,2))
end

# ╔═╡ 2455608c-c925-4b43-b0b4-30454fd908b7
# Compare vs first-order Taylor
let ϕ̂ex(x) = ϕex(x0) + ∇ϕex(x0)'*(x-x0)
	plot_vs_approx(ϕex, ϕ̂ex, x0, xx, xx)
end

# ╔═╡ 2132b2d8-54c7-416d-97c8-b00ff9ee30b3
# Compare vs second-order Taylor
let ϕ̂ex(x) = ϕex(x0) + ∇ϕex(x0)'*(x-x0) + 0.5*(x-x0)'*Hϕex(x0)*(x-x0)
	plot_vs_approx(ϕex, ϕ̂ex, x0, xx, xx)
end

# ╔═╡ 589d4dc4-ae38-11ec-1b67-518c4ac76391
md"""
If $\nabla \phi(x) \neq 0$ then $\nabla \phi(x)$ and $-\nabla \phi(x)$
...
# ╔═╡ c1352936-429c-4520-9b9a-a23f4fd1f962
let x = [0.0; 0.5]
	...
	p1 = plot(xx, xx, (x,y)->ϕex([x; y]), st=:contour)
...
# ╔═╡ cf7a224f-c734-4c05-800b-d7e6ac9c73a1
md"""
...
# ╔═╡ d9f7ab98-ba7e-4f0e-8136-b9ac3c18bc46
md"""
#### Questions
...
4.  Let $p$ be a Newton update starting from $x$, and let $w = \phi(x)-\phi(x+p)$ be the improvement in the function value during that Newton step.  Show that $\left| w-\frac{1}{2} p^T H_{\phi}(x) p \right| \leq \frac{M}{2} \|p\|^3$.
"""

# ╔═╡ 2c270c58-fc14-4797-a751-c752e0e40823
md"""
## Gradient descent
...
# ╔═╡ 7c49c75f-c395-4ca4-831b-388840dd0cd8
let x = [0.0; 0.5], α=0.1
...
# ╔═╡ 03364297-45ec-4562-b368-605456fce0ab
md"""
...
Our example problem is not quadratic, of course.  But close to the minimum at $(0,0)$, it is close enough to quadratic that we get a good picture of the convergence from the quadratic approximation.  Try plugging in the optimal $\alpha$ and some nearby values in this case to confirm that it gives about as good convergence as we can manage.
"""

# ╔═╡ 04148b83-e868-463e-aa0b-edc73f03cee4
let A = Hϕex([0; 0])
	λs = eigvals(A)
	αref = 2/(λs[1]+λs[2])
	ρref = 1-2/(1+cond(A))
...
let A = [1.0 0.0; 0.0 1e-2], x = [1.0; 1.0]
	...
	# Plot the function and the iterates
	p1 = plot(xx, xx, (x,y)->0.5*[x; y]'*A*[x; y], st=:contour)
	plot!([x[1] for x in xs], [x[2] for x in xs], marker=true, label=false)
...
# ╔═╡ 7d416e4d-f380-4a5d-b513-140d4cd7b775
md"""
...
# ╔═╡ 01c80ecf-a749-4b3d-9f19-060d0618c665
md"""
## "Steepest" descent

At a point $x$, a direction $p$ is a *descent* direction for $\phi$ if 
$\phi'(x) p < 0$.  We measure how "steep" a descent direction is by the rate
at which the function value decreases if we move at unit speed in that direction:
$-\phi'(x) p/\|p\|$.  If $\|\cdot\|$ is the ordinary Euclidean norm
on $\mathbb{R}^n$, we have that the direction of steepest descent
is $-\phi'(x)^T/\|\phi'(x)\|$.  But what happens if
we use a different norm for measuring distance?
...
Of course, these are not the only norms out there.  In particular, if $M$ is a
positive definite matrix, then there is an associated inner product
...
# ╔═╡ ffa01dd6-0917-4bf5-85dd-ecd92f0934a7
md"""
...
# ╔═╡ 0c7b89b3-1313-474e-8e74-1e08a446816c
let x = [0.0; 0.5], α=1.0
...
# ╔═╡ 6e0f3a1b-429a-4755-a015-ed5014de3194
md"""
...
# ╔═╡ c2932fa2-3f36-4186-a33f-a09065fa4dae
md"""
#### Questions

Suppose $\phi$ is a $C^2$ function and $H_{\phi}$ is Lipschitz with constant $M$.
Let $x_*$ be a strong local minimizer for $\phi$ and consider the scaled steepest descent iteration
...
Therefore, the iteration converges quadratically for good enough initial guesses.
Can you also give a condition on $\|e_0\|$ that guarantees an initial guess is
"good enough"?
"""
